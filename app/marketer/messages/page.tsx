...
export default function ChatDashboard() {
	...
	const [localUser, setLocalUser] = useState<any>(loggedInUser);
	...
	useEffect(() => {
		if (loggedInUser) {
			...
			if (userCookie) {
				try {
					setLocalUser(JSON.parse(userCookie));
				} catch (e) {
	...
	useEffect(() => {
		const run = async () => {
			try {
				...
				if (rid) {
					const res = await fetchPeerId(rid, true);
	...
	useEffect(() => {
		const fetchConversations = async () => {
			try {
				...
				// Handle different response structures (array or object with data/conversations property)
				const data = Array.isArray(response)
					? response
					: (Array.isArray(response?.data) ? response.data : (Array.isArray(response?.conversations) ? response.conversations : []));
	...
	useEffect(() => {
				...
				if (!rid && Array.isArray(conversations)) {
					...
					const other = Array.isArray(conv?.participants)
						? conv.participants.find((p: any) => {
							...
							return myId && pId && String(pId) !== String(myId);
						})
						: null;
		...
		return () => { };
	}, [conversationId, conversations, localUser]);
	...
	const transformedContacts = conversations?.map((contact: any) => {
		const otherParticipant = contact?.participants?.find((p: any) => {
			const myId = localUser?._id || localUser?.id;
			const pId = p?._id || p?.id;
			// Ensure both IDs exist and are not equal
			return myId && pId && String(pId) !== String(myId);
		});
		...
		<div className="flex h-screen bg-[#F5F5F5] overflow-hidden">
			<aside className="w-64 bg-white border-r border-[#E5E7EB] flex flex-col">
				...
				<nav className="flex-1 p-4 space-y-2">
					<Link
						href="/my-profile"
						onClick={(e) => { e.preventDefault(); window.location.href = "/my-profile"; }}
						className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#6B7280] hover:bg-[#F9FAFB] transition-colors"
					>
					...
					<Link
						...
						onClick={(e) => { e.preventDefault(); window.location.href = "/marketer/client"; }}
						className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#6B7280] hover:bg-[#F9FAFB] transition-colors"
					>
					...
					<Link
						...
						className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[#9AC63F] text-white cursor-default transition-colors"
					>
					...
					<Link
						...
						onClick={(e) => { e.preventDefault(); window.location.href = "/marketer/get-assessment"; }}
						className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#6B7280] hover:bg-[#F9FAFB] transition-colors"
					>
					...
					<Link
						...
						onClick={(e) => { e.preventDefault(); window.location.href = "/marketer/see-therapist"; }}
						className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#6B7280] hover:bg-[#F9FAFB] transition-colors"
					>
										...
										<div className="relative h-8 w-8 rounded-full overflow-hidden">
											<img
												src={(function () {
													...
													const u = String((localUser?.avatarUrl || localUser?.avatar) ?? "").trim();
													if (!u) return "/images/avatar.PNG";
							...
							<div className="bg-gray-100 border-b border-gray-200 p-4">
								<div className="flex items-center justify-between">
									<div className="flex items-center space-x-3">
										...
										/>
										<div onClick={() => console.log(selectedContact)}>
