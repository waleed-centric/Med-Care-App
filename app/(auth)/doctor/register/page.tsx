...
export default function RegisterDoctorDemo() {
	...
	const handleTimeChange = (field: "from" | "to", value: string) => {
		// Simple masking/validation for HH:MM could go here
		setFormData((prev) => ({
			...prev,
			availabilityTime: { ...prev.availabilityTime, [field]: value },
		}));
	};

	const togglePeriod = (field: "fromPeriod" | "toPeriod") => {
		setFormData((prev) => ({
			...prev,
			availabilityTime: {
				...prev.availabilityTime,
				[field]: prev.availabilityTime[field] === "AM" ? "PM" : "AM",
			},
		}));
	};
	...
	const handleSubmit = async () => {
		...
		if (formData.availabilityDays.length === 0) errs.availabilityDays = "Select at least one day";
		if (!formData.availabilityTime.from) errs.availabilityTime = "Enter start time";
		if (!formData.availabilityTime.to) errs.availabilityTime = "Enter end time";
		...
		try {
			const payload = {
				...
				availabilityFrom: `${formData.availabilityTime.from} ${formData.availabilityTime.fromPeriod}`,
				availabilityTo: `${formData.availabilityTime.to} ${formData.availabilityTime.toPeriod}`,
						...
						<div className="space-y-6">
							...
							<div className="grid grid-cols-2 gap-6 pt-4">
								...
								<div>
									...
									<p className="text-xs text-gray-500 mb-2">Select the available days</p>
										...
										{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => {
											...
											return (
												<button
													...
													className={`w-10 h-10 rounded-lg text-sm font-medium transition-colors border ${isSelected
														? "bg-[#FFEAD1] text-[#E58A13] border-[#E58A13]"
									...
									{fieldErrors.availabilityDays && (
										<p className="text-red-600 text-xs mt-1">{fieldErrors.availabilityDays}</p>
									)}
								</div>
								...
								<div>
									...
									<div className="flex gap-4 items-center">
										...
										<div>
											...
											<div className="flex border border-gray-300 rounded-lg overflow-hidden space-x-0">
												<input
													autoComplete="off"
													type="text"
													value={formData.availabilityTime.from}
													onChange={(e) => handleTimeChange("from", e.target.value)}
													placeholder="08:30"
													className="w-16 px-2 py-2 text-center text-sm focus:outline-none"
												/>
												<button
													type="button"
													onClick={() => togglePeriod("fromPeriod")}
													className={`px-3 py-2 text-sm font-medium ${formData.availabilityTime.fromPeriod === "AM" ? "bg-[#F7931E] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
												>
													AM
												</button>
												<button
													type="button"
													onClick={() => togglePeriod("fromPeriod")}
													className={`px-3 py-2 text-sm font-medium ${formData.availabilityTime.fromPeriod === "PM" ? "bg-[#F7931E] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
												>
													PM
												</button>
											</div>
										...
										<div>
											...
											<div className="flex border border-gray-300 rounded-lg overflow-hidden space-x-0">
												<input
													autoComplete="off"
													type="text"
													value={formData.availabilityTime.to}
												...
												<button
													type="button"
													onClick={() => togglePeriod("toPeriod")}
													className={`px-3 py-2 text-sm font-medium ${formData.availabilityTime.toPeriod === "AM" ? "bg-[#F7931E] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
												>
													AM
												</button>
												<button
													type="button"
													onClick={() => togglePeriod("toPeriod")}
													className={`px-3 py-2 text-sm font-medium ${formData.availabilityTime.toPeriod === "PM" ? "bg-[#F7931E] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
												>
													PM
												</button>
											</div>
									...
									{fieldErrors.availabilityTime && (
										<p className="text-red-600 text-xs mt-1">{fieldErrors.availabilityTime}</p>
									)}
								</div>
							</div>
